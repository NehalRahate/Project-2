...
function activate(_) {
  ...
  function formatTerminalContent(content) {
			...
			<body>
				...
				(function() {
					...
					function showStarterText(){
						...
						let $el = $('<div class="code-block-input chatMessage recieved starter-text"><div class="by">BLACKBOX</div><div class="chatMessage-text">Hello, Blackbox AI Chat can answer coding questions. Below are recommendations to get the best out of BLACKBOX</div><ul>'+recommendations+'</ul><div class="chat-button-containber"><div style="cursor:pointer" class="btn-walkthrough vscode-button white w-button">Discover More Features</div></div><br></div>')
	...
	vscode.commands.registerCommand('extension.saveCodeToCloud', async() => {
		let currentDocument = vscode.window.activeTextEditor;
		let selectedText = getSelectedText()
		let fileContent = ''
		if (selectedText.length == 0) fileContent = currentDocument.document.getText();
		else fileContent = selectedText
		let absPath = currentDocument.document.uri.path
		let fileName = currentDocument.document.fileName;
		let language = currentDocument.document.languageId;
